...
export function BesiktningFormModal({
  fastighetId,
  tekniskObjektId,
  onClose,
  onSaved,
  ...
  const [forfallodatum, setForfallodatum] = useState('')
  const [aterkommande, setAterkommande] = useState(false)
  const [intervallManader, setIntervallManader] = useState('12')
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const { error } = await supabase.from('besiktningar').insert({
      ...
      typ,
      forfallodatum: forfallodatum || null,
      aterkommande,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 px-4">
      ...
      >
        <h2 className="mb-4 font-display text-lg font-semibold">Ny besiktning</h2>
        <label className="mb-3 block">
          ...
          <input
            ...
            className="input"
            placeholder="t.ex. OVK, Brandskyddskontroll"
          />
        </label>
        <label className="mb-3 block">
          ...
          <input
            type="date"
            value={forfallodatum}
            onChange={(e) => setForfallodatum(e.target.value)}
            className="input"
          />
        </label>
