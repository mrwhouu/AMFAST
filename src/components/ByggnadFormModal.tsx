...
export function ByggnadFormModal({
  fastighetId,
  onClose,
  onSaved,
}: {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const { error } = await supabase
      .from('byggnader')
      .insert({ fastighet_id: fastighetId, namn, beskrivning: beskrivning || null })
    setSaving(false)
    if (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 px-4">
      ...
      >
        <h2 className="mb-4 font-display text-lg font-semibold">Ny byggnad</h2>
